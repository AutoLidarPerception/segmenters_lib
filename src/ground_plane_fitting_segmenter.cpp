...
namespace segmenter {
    ...
    GroundPlaneFittingSegmenter::GroundPlaneFittingSegmenter(const SegmenterParams& params) :
            params_(params)
    {}
     ...
     *      ==> guarantees that noisy measurements will not affect the plane estimation step
     * @input
     *  params_.gpf_num_lpr: number of  lowest point representative(LPR),
     * @param cloud_in
     * @param cloud_seeds
     */
    void GroundPlaneFittingSegmenter::extractInitialSeeds(const PointICloud& cloud_in,
                                                          PointICloudPtr cloud_seeds)
    {
        ...
        //the points inside the height threshold are used as the initial seeds for the plane model estimation
        (*cloud_seeds).clear();
     ...
     */
    model_t GroundPlaneFittingSegmenter::estimatePlane(const PointICloud& cloud_ground)
    {
        model_t model;
        ...
        for (int i = 0; i < cloud_ground.points.size(); i++) {
            cov_xx += (cloud_ground.points[i].x - mean_x) * (cloud_ground.points[i].x - mean_x);
            cov_xy += (cloud_ground.points[i].x - mean_x) * (cloud_ground.points[i].y - mean_y);
            cov_xz += (cloud_ground.points[i].x - mean_x) * (cloud_ground.points[i].z - mean_z);
            cov_yy += (cloud_ground.points[i].y - mean_y) * (cloud_ground.points[i].y - mean_y);
            cov_yz += (cloud_ground.points[i].y - mean_y) * (cloud_ground.points[i].z - mean_z);
            cov_zz += (cloud_ground.points[i].z - mean_z) * (cloud_ground.points[i].z - mean_z);
        }
        ...
        // d is directly computed substituting x with s^ which is a good representative for the points belonging to the plane
    ...
    void GroundPlaneFittingSegmenter::mainLoop(const PointICloud& cloud_in,
                                               PointICloudPtr cloud_gnds, PointICloudPtr cloud_ngnds)
    {
        cloud_gnds->clear();
        cloud_ngnds->clear();
    ...
    void GroundPlaneFittingSegmenter::segment(const PointICloud& cloud_in,
                                              std::vector<PointICloudPtr>* cloud_clusters)
    {
        if (cloud_clusters == nullptr || cloud_in.empty()) {
            return;
        }
        //Clear segments.
        ...
        //Main Loop
        PointICloudPtr cloud_ground(new PointICloud), cloud_nonground(new PointICloud);
        if (params_.gpf_num_segment > 1) {
            ...
            const double res = ((--points.end())->x - points.begin()->x) / params_.gpf_num_segment;
            ...
            for (size_t segmentIdx = 0u; segmentIdx < params_.gpf_num_segment; ++segmentIdx) {
        ...
        ROS_INFO_STREAM("GPF Segmentation complete. Took " << clock.takeRealTime() << "ms.");
    }
}